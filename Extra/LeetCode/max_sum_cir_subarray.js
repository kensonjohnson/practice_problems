...
var maxSubarraySumCircular = function (nums) {
  ...
  let highestFound = Number.NEGATIVE_INFINITY;
  // We let i determine the size of the window
  for (let i = 1; i <= n; i++) {
    // create n windows
    for (let j = 0; j < n; j++) {
      // Walk all the elements in current window, summing them
      let window = i;
      let total = 0;
      while (window) {
        total += nums[(j + window) % n];
        window--;
      }
      // if total is higher than highestFound, save it
