...
const testRooms1 = [[1], [2], [3], []]; // Expect true
const testRooms2 = [[1, 3], [3, 0, 1], [2], [0]]; // Expect false
...
var canVisitAllRooms = function (rooms) {
  ...
  while (toVisit.length > 0) {
    const currentRoom = rooms[toVisit.pop()];
    currentRoom.forEach((key) => {
      if (!roomsVisited.has(key)) {
        roomsVisited.add(key);
        toVisit.push(key);
      }
    });
  }
...
console.log(canVisitAllRooms(testRooms1));
console.log(canVisitAllRooms(testRooms2));
